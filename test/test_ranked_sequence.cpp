#include "common.hpp"
#include "include/ranked_sequence.hpp"

constexpr uint64_t sequence_length = 10000;

ranked_sequence encode_with_ranked_sequence(std::vector<uint64_t> const& seq) {
    std::cout << "encoding seq with a ranked_sequence..." << std::endl;
    ranked_sequence rs;
...
TEST_CASE("access") {
    ...
    auto rs = encode_with_ranked_sequence(seq);
...
TEST_CASE("save_mmap_ranked_sequence") {
    uint64_t max_small_value = test::get_random_uint() % 5;
    if (max_small_value == 0) max_small_value = 1;
    uint64_t max_large_value = test::get_random_uint() % 100000;
    if (max_large_value == 0) max_large_value = 100;
    ...
    std::vector<uint64_t> seq =
        test::get_skewed_sequence(sequence_length, max_small_value, max_large_value, 0.98);
    const std::string output_filename("rs.bin");
    ...
    {
        auto rs = encode_with_ranked_sequence(seq);
        num_saved_bytes = essentials::save(rs, output_filename.c_str());
        std::cout << "num_saved_bytes = " << num_saved_bytes << std::endl;
    }

    {
        ranked_sequence rs_mmapped;
        num_mapped_bytes = essentials::mmap(rs_mmapped, output_filename.c_str());
        std::cout << "num_mapped_bytes = " << num_mapped_bytes << std::endl;
        REQUIRE(num_saved_bytes == num_mapped_bytes);

        std::cout << "checking correctness of access..." << std::endl;
        for (uint64_t i = 0; i != seq.size(); ++i) {
            uint64_t got = rs_mmapped.access(i);
